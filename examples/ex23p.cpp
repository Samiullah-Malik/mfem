...
//
// Sample runs:  mpirun -np 4 ex23p -o 2 -f 1.3 -rs 1 -rp 1 -prob 0
//               mpirun -np 4 ex23p -o 2 -f 1.3 -rs 1 -rp 1 -prob 1
//               mpirun -np 4 ex23p -o 2 -f 3.3 -rs 3 -rp 1 -prob 2
//               mpirun -np 4 ex23p -o 2 -f 1.3 -rs 2 -rp 1 -prob 3
//               mpirun -np 4 ex23p -o 2 -f 10.3 -rs 2 -rp 2 -m ../data/inline-quad.mesh
//               mpirun -np 4 ex23p -o 2 -f 2.3 -rs 1 -rp 1 -m ../data/inline-hex.mesh
...
double detJ_inv_Re(const Vector &x, CartesianPML * pml);
double detJ_inv_Im(const Vector &x, CartesianPML * pml);
...
void GetMaxwellPMLCoefficients2D(CartesianPML *, Coefficient *&, Coefficient *&,
                                 MatrixCoefficient *&, MatrixCoefficient *&);
void GetMaxwellPMLCoefficients3D(CartesianPML *, MatrixCoefficient *&,
                                 MatrixCoefficient *&,
                                 MatrixCoefficient *&, MatrixCoefficient *&);
...
int main(int argc, char *argv[])
{
   ...
   if (pmesh->attributes.Size() && pmesh->attributes.Max() > 1) // setpml attribute
   {
      attr = 0;
      attr[1] = 1;
   }

   MatrixCoefficient * pml_c2_Re=nullptr;
   MatrixCoefficient * pml_c2_Im=nullptr;
   if (dim == 3)
   {
      MatrixCoefficient * pml_c1_Re=nullptr;
      MatrixCoefficient * pml_c1_Im=nullptr;
      MatrixCoefficient * c1_Re=nullptr;
      MatrixCoefficient * c1_Im=nullptr;

      GetMaxwellPMLCoefficients3D(pml, pml_c1_Re, pml_c1_Im, pml_c2_Re, pml_c2_Im);

      c1_Re = new ScalarMatrixProductCoefficient(muinv,*pml_c1_Re);
      c1_Im = new ScalarMatrixProductCoefficient(muinv,*pml_c1_Im);

      MatrixRestrictedCoefficient * coeff3D_Re = new MatrixRestrictedCoefficient(
         *c1_Re,attr);
      MatrixRestrictedCoefficient * coeff3D_Im = new MatrixRestrictedCoefficient(
         *c1_Im,attr);

      a.AddDomainIntegrator(new CurlCurlIntegrator(*coeff3D_Re),
                            new CurlCurlIntegrator(*coeff3D_Im));
   }
   else
   {
      Coefficient * pml_c1_Re=nullptr;
      Coefficient * pml_c1_Im=nullptr;
      Coefficient * c1_Re=nullptr;
      Coefficient * c1_Im=nullptr;
      GetMaxwellPMLCoefficients2D(pml, pml_c1_Re, pml_c1_Im, pml_c2_Re, pml_c2_Im);

      c1_Re = new ProductCoefficient(muinv,*pml_c1_Re);
      c1_Im = new ProductCoefficient(muinv,*pml_c1_Im);

      RestrictedCoefficient  * coeff2D_Re = new RestrictedCoefficient(*c1_Re,attr);
      RestrictedCoefficient  * coeff2D_Im = new RestrictedCoefficient(*c1_Im,attr);

      a.AddDomainIntegrator(new CurlCurlIntegrator(*coeff2D_Re),
                            new CurlCurlIntegrator(*coeff2D_Im));
   ...
   c2_Re = new ScalarMatrixProductCoefficient(sigma,*pml_c2_Re);
   c2_Im = new ScalarMatrixProductCoefficient(sigma,*pml_c2_Im);

   MatrixRestrictedCoefficient *pml_coeff_Re = new MatrixRestrictedCoefficient(
      *c2_Re,attr);
   MatrixRestrictedCoefficient *pml_coeff_Im = new MatrixRestrictedCoefficient(
      *c2_Im,attr);
...
void GetMaxwellPMLCoefficients2D(CartesianPML * pml, Coefficient *& c1_Re,
                                 Coefficient *& c1_Im,
                                 MatrixCoefficient *& c2_Re, MatrixCoefficient *& c2_Im)
{
   c1_Re = new PmlCoefficient(detJ_inv_Re, pml);
   c1_Im = new PmlCoefficient(detJ_inv_Im, pml);
   c2_Re = new PmlMatrixCoefficient(dim, detJ_JT_J_inv_Re, pml);
   c2_Im = new PmlMatrixCoefficient(dim, detJ_JT_J_inv_Im, pml);
}

void GetMaxwellPMLCoefficients3D(CartesianPML * pml, MatrixCoefficient *& c1_Re,
                                 MatrixCoefficient *& c1_Im,
                                 MatrixCoefficient *& c2_Re, MatrixCoefficient *& c2_Im)
{
   c1_Re = new PmlMatrixCoefficient(dim, detJ_inv_JT_J_Re, pml);
   c1_Im = new PmlMatrixCoefficient(dim, detJ_inv_JT_J_Im, pml);
   c2_Re = new PmlMatrixCoefficient(dim, detJ_JT_J_inv_Re,pml);
   c2_Im = new PmlMatrixCoefficient(dim, detJ_JT_J_inv_Im,pml);
}
